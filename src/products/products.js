	...
	try {
		...
		if (req.query.category) {
			...
			filteredProducts.length > 0
				...
				: next(
						createError(404, `No products with category: ${req.query.category}`)
				  );
		}
		if (req.query.name) {
			// ?name=filtered
			...
			products.length > 0
				? res.send(products)
				: next(createError(404, "No products available!"));
		}
	} catch (error) {
		next(error);
	}
...
productsRouter.get("/:id", async (req, res, next) => {
	try {
		...
		result
			...
			: next(
					createError(404, "Product not found, check your ID and try again!")
			  );
	} catch (error) {
		next(error);
	}
...
productsRouter.post("/", async (req, res, next) => {
	try {
		...
		product
			? res.send(product)
			: next(createError(400, "Error creating product, try again!"));
	} catch (error) {
		next(error);
	}
});
			...
			if (!req.file) next(createError(400, "What file exactly?"));
				...
				if (!result) next(createError(400, "id does not match"));
					...
					product.image = `${req.protocol}://${req.get(
						"host"
					)}/images/productImages/${req.params.id}${path.extname(
...
productsRouter.get("/:id/reviews", async (req, res, next) => {
	try {
		...
		result
			...
			: next(createError(404, "id does not match"));
	} catch (error) {
		next(error);
	}
});
